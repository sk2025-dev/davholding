...
export default function PromoSlides() {
  ...
  const openEdit = (slide) => {
    ...
    setForm({
      ...
      link_label: slide.link_label || "En savoir plus",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.link_label)  fd.append("link_label", form.link_label);
      fd.append("sort_order", form.sort_order);
      fd.append("is_active", form.is_active ? "1" : "0");
      if (form.image) fd.append("image", form.image);

      if (editSlide) {
        await adminApi.updatePromoSlide(editSlide.id, fd);
        showToast("Slide mise à jour.", 2500);
      } else {
        await adminApi.createPromoSlide(fd);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await adminApi.deletePromoSlide(slide.id);
  ...
  const toggleActive = async (slide) => {
    try {
      ...
      await adminApi.updatePromoSlide(slide.id, fd);
      await load();
    } catch (err) {
      showToast(err.message, 3000);
    }
  ...
  return (
    <div className="panel-wrap">
          ...
          {slides.map((slide) => (
            <div key={slide.id} className={`promo-slide-card${!slide.is_active ? " promo-slide-card--inactive" : ""}`}>
              ...
              <div className="promo-slide-body">
                <p className="promo-slide-order">#{slide.sort_order}</p>
                <h3>{slide.title}</h3>
                {slide.subtitle && <p className="promo-slide-sub">{slide.subtitle}</p>}
                {slide.description && <p className="promo-slide-desc">{slide.description}</p>}
                {slide.link_url && (
                  <p className="promo-slide-link">
                    🔗 <a href={slide.link_url} target="_blank" rel="noreferrer">{slide.link_label}</a>
                  </p>
                )}
              </div>
        ...
        <div className="modal-overlay open" onClick={(e) => e.target === e.currentTarget && closeModal()}>
          <div className="modal-box" style={{ maxWidth: 560 }}>
            ...
            <form className="modal-form" onSubmit={handleSubmit}>
              ...
              <label className="form-label">
                Sous-titre
                <input name="subtitle" className="form-input" value={form.subtitle} onChange={handleChange} />
              </label>
              <label className="form-label">
                Description
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <label className="form-label">
                  Lien URL
                  <input name="link_url" className="form-input" type="url" value={form.link_url} onChange={handleChange} placeholder="https://…" />
                </label>
                <label className="form-label">
                  Texte du bouton
                  <input name="link_label" className="form-input" value={form.link_label} onChange={handleChange} />
                </label>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                <label className="form-label">
      ...
      <style>{`
        ...
        .promo-slide-link { font-size: 12px; margin: 0; }
        .promo-slide-actions {
          display: flex;
          gap: 8px;
