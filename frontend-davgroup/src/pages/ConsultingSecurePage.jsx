...
const FILTERS_BASE = [{ key: "all", label: "Tous" }];
...
function DesignCard({ item, delay, onOpen }) {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting) { el.classList.add("ds-visible"); observer.unobserve(el); } },
...
/* ════════════════════════════════════ */
export default function ConsultingSecurePage() {
  const [items, setItems] = useState([]);
  const [lightboxItem, setLightboxItem] = useState(null);
  const [filter, setFilter] = useState("all");
  const heroRef = useRef(null);

  const categories = [...new Set(items.map((p) => p.cat).filter(Boolean))];
  const filters = [...FILTERS_BASE, ...categories.map((c) => ({ key: c, label: c }))];
  ...
  useEffect(() => {
    fetch(`${API_URL}/consulting-realisations?category=secure`)
      .then((res) => res.json())
      .then((json) => {
  ...
  return (
    <ConsultingLayout hideNav>
      ...
      <section className="ds-hero">
        <img className="ds-hero-bg" src="/consulting/images/enneu.png" alt="" />
        <div className="ds-hero-overlay" />

        <Link to="/consulting" className="ds-logo">
          <img src="/consulting/images/code.png" alt="DAV Consulting" />
        </Link>

        <Link to="/consulting" className="ds-back-btn">
        ...
        <div className="ds-hero-content" ref={heroRef}>
          <h1 className="ds-hero-title">Sécurisez votre infrastructure.</h1>
      ...
      <section className="ds-section">
        <div className="ds-section-header">
          <div className="ds-section-title">Nos réalisations</div>
